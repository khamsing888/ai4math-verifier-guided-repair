...
| 2 | **Lâm Thành Trung** | **B25CHHT117** | **Queue & Worker System** (TV5) | Evaluation & Cost Analysis | • Xây dựng kiến trúc hàng đợi phân tán (Queue) và quản lý worker pools.<br>• Thiết kế vòng lặp sửa có chặn (Bounded retry loop), cơ chế timeout.<br>• Đo lường và tối ưu hóa độ trễ đuôi Queue P95. |
| 3 | **Nguyễn Xuân Tùng** | **B25CHHT119** | **LLM Repair Engine** (TV4) | Rule-based Repair | • Thiết kế Prompt Templates tiêm ngữ cảnh lỗi có cấu trúc (Diagnostic Injection).<br>• Tích hợp API/Model mã nguồn mở sửa các lỗi logic phức tạp (Type mismatch, Unsolved goals).<br>• Tối ưu hóa số lượng token tiêu thụ trên mỗi lần sửa. |
| 4 | **Trần Quang Đức Dũng** | **B25CHHT088** | **Rule-based Repair** (TV3) | LLM Repair Engine | • Phát triển các bộ luật heuristic sửa nhanh (0-Token cost fixes): cân bằng ngoặc, sửa lỗi gõ cú pháp.<br>• Tự động bổ sung namespace và tra cứu thư viện Mathlib import.<br>• Đánh giá tỷ lệ bao phủ của luật (Rule coverage). |
...
| **W2** | Nhận Data Contract từ Nhóm 5, dựng Replayable Error Store | Nounou, Khamsing | - | 🔄 Đang triển khai | Data contract schema v0.1 + script replay lỗi |
| **W3** | Chạy Baseline retry nguyên prompt trên dữ liệu đầy đủ | Dũng, Tùng | - | ⏳ Chờ thực hiện | Baseline raw metrics + demo nội bộ |
| **W4** | Hoàn thiện Error Parser & Rule-based fixes cho lỗi đơn giản | Khamsing, Dũng | - | ⏳ Chờ thực hiện | Parser accuracy test + Rule coverage report |
| **W5** | Hoàn thiện Router và module LLM Repair nạp ngữ cảnh lỗi | Tùng, Tâm | - | ⏳ Chờ thực hiện | Repair service v0.1 + prompt versions |
| **W6** | Tích hợp Bounded Loop, Timeout, Dedup & Queue workers | Trung, Tâm | - | ⏳ Chờ thực hiện | End-to-end pipeline service + recovery test |
| **W7** | Thí nghiệm Scalability 3 mức tải (100 $\rightarrow$ 1K $\rightarrow$ 10K) | Trung, Tâm, Cả nhóm | - | ⏳ Chờ thực hiện | Raw CSV/JSON + Biểu đồ Queue P95 & Throughput |
